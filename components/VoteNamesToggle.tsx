...
export default function VoteNamesToggle({ yeas, nays, abstains }: Props) {
  ...
  return (
    <details className="group border-t border-slate-100">
      <summary className="list-none marker:hidden [&::-webkit-details-marker]:hidden cursor-pointer w-full px-4 py-2.5 text-xs text-slate-500 hover:text-slate-700 hover:bg-slate-50 transition-colors flex items-center justify-between">
        <span className="group-open:hidden">Show all {total} voter names</span>
        <span className="hidden group-open:inline">Hide individual votes</span>
        <svg
          className="w-3.5 h-3.5 transition-transform duration-150 group-open:rotate-180"
